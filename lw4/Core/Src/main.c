...
char CardID[4*3+1];               // серійний номер картки (символьний)
...
const uint8_t snOn[4] = {0x67, 0xE9, 0x01, 0x68};
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, status != MI_OK);
    if (status != MI_OK)
    {

      HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, GPIO_PIN_SET);
      sprintf(CardID, "No card");
      LCD_Clear();
      LCD_String(CardID);
    }
    ...
    {
      sprintf(CardID, "%X %X %X %X", sn[0], sn[1], sn[2], sn[3]);
      LCD_Clear();
      LCD_String(CardID);
      for (int i = 0; i < 4; i++){
        if (sn[i] != snOn[i]) break;
        if (i == 3)           LCD_String(" +");
      }
      HAL_Delay(500);
    }
    // Вибір картки
    MFRC522_SelectTag(sn);
    // Якщо запис дозволено
    if (write_on)
    {
      // Авторизація для запису
      blockAddr = 3; // звернення до блоку 3 для авторизації в секторі 0
      status = MFRC522_Auth(PICC_AUTHENT1A, blockAddr, sectorKeyA[0], sn);
      if (status == MI_OK)
      {
        // Запис даних до блоку 1
        blockAddr = 1;
    ...
    if (status == MI_OK)
    {
      ...
      if (status == MI_OK)
      {
        sprintf(DataChar[0], "%02X%02X%02X%02X%02X%02X%02X%02X",
                str[0], str[1], str[2], str[3], str[4], str[5], str[6], str[7]);
        sprintf(DataChar[1], "%02X%02X%02X%02X%02X%02X%02X%02X",
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, LCD_CS_Pin | LCD_SID_Pin | LCD_SCK_Pin | LCD_RS_Pin | LCD_E_Pin | LCD_D4_Pin | LCD_D5_Pin | LCD_D6_Pin | LCD_D7_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : LCD_CS_Pin LCD_SID_Pin LCD_SCK_Pin LCD_RS_Pin
                           ...
                           LCD_D7_Pin */
  GPIO_InitStruct.Pin = LCD_CS_Pin | LCD_SID_Pin | LCD_SCK_Pin | LCD_RS_Pin | LCD_E_Pin | LCD_D4_Pin | LCD_D5_Pin | LCD_D6_Pin | LCD_D7_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
